                    ...
                    @if (Cart::instance('cart')->count() > 0)
                        ...
                        <ul class="products-cart">
                            @foreach (Cart::instance('cart')->content() as $item)
                                {{-- @foreach (Cart::instance('cart')->instance('cart')->content()
    as $item) --}}
                                ...
                                <li class="pr-cart-item">
                                    {{-- new --}}
                                    {{-- @php
                                        $selected_product=App\Models\Product::find($item->id);
                                        if(is_null($selected_product)) {
                                            Cart::instance('cart')->instance('cart')->remove($item->rowId);
                                        }
                                    @endphp --}}
                                    ...
                                    @foreach ($item->options as $key => $value)
                                        <div style="vertical-align:middl; width=180px">
                                            <p><b>{{ $key }}: {{ $value }}</b></p>
                                        </div>
                                    @endforeach
                    ...
                    @foreach ($productss as $productss)
                        <div class="product product-style-2 equal-elem ">
                            <div class="product-thumnail">
                                <a href="{{ route('product.details', ['slug' => $productss->slug]) }}"
                                    title="{{ $productss->name }}">
                                    <figure><img
                                            src="{{ asset('assets/images/products') }}/{{ $productss->image }}"
                                            width="214" height="214" alt="{{ $productss->name }}"></figure>
                                </a>
                            ...
                            <div class="product-info">
                                <a href="{{ route('product.details', ['slug' => $productss->slug]) }}"
                                    class="product-name"><span>{{ $productss->description }}</span></a>
                                <div class="wrap-price"><span
                                        class="product-price">${{ $productss->regular_price }}</span></div>
                            </div>
                        </div>
                    @endforeach
